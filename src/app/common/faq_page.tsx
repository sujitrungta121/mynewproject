...
export default function FAQ({backgroundColor} : {backgroundColor:String}){
  ...
  <div
    key={item.question}
    className={`flex flex-row xl:justify-between lg:justify-between md:justify-between sm:justify-between justify-between w-full items-center xl:gap-32 lg:gap-32 md:gap-32 hover:cursor-pointer ${openFaq && openIndex === item.id && "bg-lime-50 p-8"}`}style={{ backgroundColor: openFaq && openIndex === item.id && backgroundColor }}
